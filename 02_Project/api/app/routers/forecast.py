...
def stockout_risk(conn: psycopg.Connection = Depends(get_connection)) -> list[StockoutForecastOut]:
...
def detect_stockout_risk(conn: psycopg.Connection = Depends(get_connection)) -> DetectionResult:
    with conn.cursor() as cur:
        forecasts = compute_forecast(cur)
